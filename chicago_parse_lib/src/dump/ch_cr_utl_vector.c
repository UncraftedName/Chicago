...
ch_err ch_dump_utl_vec_text(ch_dump_text* dump, const char* vec_name, const ch_cr_utl_vector_restored* utl_vec)
{
    ...
    if (utl_vec->embedded_map) {
        ...
        for (size_t i = 0; i < utl_vec->n_elems; i++) {
            CH_RET_IF_ERR(ch_dump_text_printf(dump, "[%zu] "));
