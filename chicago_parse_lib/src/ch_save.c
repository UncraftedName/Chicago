...
ch_err ch_append_str_ll_vfmt(ch_arena* arena, ch_str_ll** first_err, ch_str_ll** last_err, const char* fmt, va_list va)
{
    ...
    va_end(va_tmp);
    assert(len >= 0); // you fucked up

    // alloc & write string
    ch_str_ll* next_ll;
    CH_CHECKED_ALLOC(next_ll, ch_arena_alloc(arena, sizeof(ch_str_ll) + (len < 0 ? 0 : len + 1)));
    if (len < 0) {
        next_ll->str = "ERROR ENCODING LOG STRING";
    } else {
        next_ll->str = (char*)(next_ll + 1);
        vsnprintf(next_ll->str, len + 1, fmt, va);
    }
    printf("%s\n", next_ll->str);
